...
public class CSV { 
	 ...
	 ConnectException // Error de timeout
	{
		...
		try {
		    ...
		    String line = "";
		    int numLine = 0;
		    Ventana.nombreProceso.setText("Cargando CSV...");
		    while ((line = input.readLine()) != null) {
		    	numLine++;
		    	if(!GestionTwitter.consola) {
					progress = (int)(((double)numLine/(double)totalLines)*100);
					Ventana.pBar.setValue(progress);
				}
	...
	private static void procesaCabeceras( ArrayList<Object> cabs ) {
		...
		if(GestionTwitter.consola) {
			...
			Ventana.tArea.append(cabs+"\n");
			Ventana.tArea.setCaretPosition(Ventana.tArea.getDocument().getLength());
		}
	}

	public static int numLin = 0;
	public static int progress =0;
	private static void procesaLineaDatos( ArrayList<Object> datos ) {
		UsuarioTwitter usuario =new UsuarioTwitter((String)datos.get(0),(String)datos.get(1),(ArrayList<String>)datos.get(2),(String)datos.get(3),(Long)datos.get(4),(Long)datos.get(5),(String)datos.get(6),(Long)datos.get(7),(String)datos.get(8),(ArrayList<String>)datos.get(9));
		GestionTwitter.putUsuarioPorID(usuario);
		if(!GestionTwitter.consola) {  // Saca la cabecera por consola de error
	...
	private static int getTotalLines(URL url) {
	...
	public static boolean repetidos(TreeMap<String, UsuarioTwitter> usuariosPorNick) {
		return usuariosPorNick.size()!=numLin;
	}
